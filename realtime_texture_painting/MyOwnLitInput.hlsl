...
inline void InitializeStandardLitSurfaceData(float2 uv, out SurfaceData outSurfaceData)
{
        ...
        // set draw over to coinside with the uv map
        half4 paintColor = tex2D(_DrawOver, uv).rgba;

        half4 specGloss = SampleMetallicSpecGloss(uv, albedoAlpha.a);
        half3 albedo = albedoAlpha.rgb * _BaseColor.rgb;
        // just had to switch out this line so I could draw on it with the _DrawOver value
        //outSurfaceData.albedo = albedoAlpha.rgb * _BaseColor.rgb;
        outSurfaceData.albedo = lerp(half4(albedo.rgb, outSurfaceData.alpha).rgba, float4(paintColor.rgb, 1.0).rgba, paintColor.a).rgb;
